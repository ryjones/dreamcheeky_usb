"""Driver for DreamCheeky LED Message board v0.1

Modified from Chris Grice's dcusb library
 - Author:  Chris Grice
 - Author Email:  chris@chrisgrice.com
 - License:  MIT
 - URL:  https://github.com/cgrice/dcusb
...
from dcusb import alphabet
import time


class LEDMessageBoard:

    def __init__(self, vendor_id=7476, product_id=19):
        ...
        self.device = Enumeration().find(vid=vendor_id, pid=product_id)[0]
    ...
    def push_screen(self):
        ...
        for i in range(0, len(lines), 2):
            ...
            byte_4_range = reversed(
                [str(item) for item in lines[i][0:8]]
            )
            byte_3_range = reversed(
                [str(item) for item in lines[i][8:16]]
            )
            byte_2_range = reversed(
                [str(item) for item in lines[i][16:21]]
            )
            ...
            if(i < 6):
                ...
                byte_7_range = reversed(
                    [str(item) for item in lines[i + 1][0:8]]
                )
                byte_6_range = reversed(
                    [str(item) for item in lines[i + 1][8:16]]
                )
                byte_5_range = reversed(
                    [str(item) for item in lines[i + 1][16:21]]
                )
